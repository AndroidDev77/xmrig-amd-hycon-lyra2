...
static std::mutex Lyra2_cache_mutex;
static std::mutex Lyra2_build_mutex;
static std::vector<CacheEntry> Lyra2_cache;
...
static cl_program Lyra2_build_program(const GpuContext *ctx, xmrig::Variant variant, uint64_t height, const std::string &source, const std::string &options, std::string hash)
{
    std::vector<cl_program> old_programs;
    old_programs.reserve(32);
    {
        std::lock_guard<std::mutex> g(Lyra2_cache_mutex);

        // Remove old programs from cache
        for (size_t i = 0; i < Lyra2_cache.size();)
        {
            const CacheEntry& entry = Lyra2_cache[i];
            if ((entry.variant == variant) && (entry.height + PRECOMPILATION_DEPTH < height))
            {
                LOG_DEBUG("Lyra2: program for height %" PRIu64 " released (old program)", entry.height);
                old_programs.push_back(entry.program);
                Lyra2_cache[i] = std::move(Lyra2_cache.back());
                Lyra2_cache.pop_back();
            }
            else
            {
                ++i;
    ...
    std::lock_guard<std::mutex> g1(Lyra2_build_mutex);
    ...
    cl_program program = nullptr;
    {
        ...
        for (const CacheEntry& entry : Lyra2_cache)
        {
    ...
    if (ret != CL_SUCCESS)
    {
        LOG_ERR("Lyra2: clCreateProgramWithSource returned error %s", OclError::toString(ret));
        return nullptr;
    }

    ret = OclLib::buildProgram(program, 1, &ctx->DeviceID, options.c_str());
    if (ret != CL_SUCCESS) {
        LOG_ERR("Lyra2: clBuildProgram returned error %s", OclError::toString(ret));
    ...
    if (ret != CL_SUCCESS) {
        OclLib::releaseProgram(program);
        LOG_ERR("Lyra2: wait_build returned error %s", OclError::toString(ret));
        return nullptr;
    }

    LOG_DEBUG("Lyra2: programs for heights %" PRIu64 " - %" PRIu64 " compiled", height * 10, height * 10 + 9);

    {
        std::lock_guard<std::mutex> g(Lyra2_cache_mutex);
        Lyra2_cache.emplace_back(variant, height, ctx->deviceIdx, std::move(hash), program);
    }
    return program;
}
...
cl_program Lyra2_get_program(GpuContext* ctx, xmrig::Variant variant, uint64_t height, bool background)
{
    if (background) {
        background_exec([=](){ Lyra2_get_program(ctx, variant, height, false); });
        return nullptr;
    }
    ...
    if (!offset)
    {
        LOG_ERR("Lyra2_get_program: XMRIG_INCLUDE_RANDOM_MATH not found in lyra2.cl", variant);
        return nullptr;
    }
   ...
   *for (int i = 0; i < 10; ++i)
    {
        ...
        case xmrig::VARIANT_4:
            ...
            break;
        default:
            LOG_ERR("Lyra2_get_program: invalid variant %d", variant);
            return nullptr;
        }
        ...
        const char kernel_name[] = "lyra2_N";
    ...
    char options[512] = {};
    OclCache::getOptions(xmrig::LYRA2, variant, ctx, options, sizeof(options));
...

    {
        ...
        for (const CacheEntry& entry : Lyra2_cache)
        {
            if ((entry.variant == variant) && (entry.height == height) && (entry.deviceIdx == ctx->deviceIdx) && (entry.hash == hash))
            {
                LOG_DEBUG("Lyra2: program for height %" PRIu64 " found in cache", height);
                return entry.program;
            }
        }
    }

    return Lyra2_build_program(ctx, variant, height, source, options, hash);
}
